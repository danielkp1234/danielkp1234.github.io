	...
	$('audio[controls]').before(function(){
		...
		var addTime = function() {
			...
			time.showtime = function () {
				...
				$(time).attr({'title': 'Remise à zéro<hr style="padding:0; margin:0;" />Temps: ' + (time.timesplit(song.currentTime))});
				if (!song.paused){
					$(time).html(time.timesplit(song.currentTime));
					$(time).attr({'title': 'Remise à zéro<hr style="padding:0; margin:0;" />Durée: ' + (time.timesplit(song.duration))});
				}
